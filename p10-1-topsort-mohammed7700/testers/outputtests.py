...
class CompareOutputTest(StatusTest):
    ...
    def _award_points(self):
        if self.status == Status.TIMEOUT or self.status == Status.ERROR:
            print("Konnte Ausgabe nicht vergleichen: Fehler bei er Ausführung")
            self.awarded_points = 0
            return

        user_output = self.std_output.decode("utf-8")

        if not self.comparer(user_output, self.expected_output):
            print("Benutzerausgabe stimmt nicht mit geforderter Ausgabe überein:")
            print("-- Benutzerausgabe:")
            print(user_output)
            print("-- Geforderte Ausgabe / geforderte Ausgabe in folgender Datei:")
