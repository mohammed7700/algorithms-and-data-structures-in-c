...
void bt_rotate_right(BinTreeNode** link) {
    ...
    //danke dafür dass es einfach null verständlich war,
    //wie hier im Programm mit den Pointern umgegangen wird im programm (:

    if(!w || !v) return;

    if(w->right) { v->left = w->right; }
    else { v->left = NULL; }
